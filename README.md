...
**Persistent memory** — Curated `MEMORY.md` loaded every session. Daily logs, skill-specific memories, and keyword search across all memory files. After each conversation, Fliiq automatically extracts institutional knowledge — people, ongoing topics, and decisions — and merges them into structured memory files for future sessions.
